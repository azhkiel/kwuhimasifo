...
// Ambil data produk
$stmt = $pdo->prepare("SELECT * FROM mersi_products WHERE id = ?");
$stmt->execute([$id]);
$product = $stmt->fetch();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_product'])) {
    ...
    $stmt = $pdo->prepare("UPDATE mersi_products SET name = ?, description = ?, price = ?, stock = ?, image = ? WHERE id = ?");
...
<head>
    ...
    <title>Edit Produk - Merchandise</title>
...
<body class="bg-gray-100">
    <header class="bg-white shadow-md">
        <div class="container mx-auto px-4 py-4 flex justify-between items-center">
            <div>
                <h1 class="text-2xl font-bold text-blue-600">Edit Produk Merchandise</h1>
                <nav class="mt-2">
        ...
        <div class="bg-white p-6 rounded-lg shadow-md max-w-3xl mx-auto">
            ...
            <form method="POST" action="" enctype="multipart/form-data">
                ...
                <div class="flex justify-end">
                    ...
                    <button type="submit" name="update_product" class="bg-purple-500 hover:bg-purple-600 text-white font-bold py-2 px-4 rounded">
                        Update Produk
                    </button>
                </div>
            </form>
